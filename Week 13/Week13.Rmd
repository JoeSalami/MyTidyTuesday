...
subtitle: "Alcohol COnsumption in Africa"
...
alcohol <- read_csv("https://github.com/rfordatascience/tidytuesday/blob/master/data/week13_alcohol_global.csv")
...
```{r}
plot1 <- ggplot(africa_alc_dist, aes(x= reorder(country, servings),y=servings))+
...
```{r}
plot2 <- africa_alc %>% top_n(10, beer_servings) %>%
...
```{r}
plot3 <- africa_alc %>% top_n(10, wine_servings) %>%
...
```{r}
plot4 <- africa_alc %>% top_n(10, spirit_servings) %>%
